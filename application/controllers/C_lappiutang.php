...
class C_lappiutang extends CI_Controller {
	...
	public function lihat(){
	  ...
	  $query="SELECT * FROM piutang,pelanggan where ptngPlgnId='$plgnId' and ptngTanggal BETWEEN '$daritanggal' and '$hinggatanggal' and plgnId='$plgnId'";
	  $data=array(
	...
	public function cetak($plgnId,$dari,$hingga){
	  $plgnId=$plgnId;
	  $daritanggal=$dari;
	  ...
	  $data=array(
			...
			'daritanggal'=>$daritanggal,
			'hinggatanggal'=>$hinggatanggal,
			'pelanggan'=>$this->M_pos->kueri("SELECT * FROM pelanggan WHERE plgnId='$plgnId'")->row(),
		);

	  $this->load->view('lappiutang/cetak',$data);
	}
}
